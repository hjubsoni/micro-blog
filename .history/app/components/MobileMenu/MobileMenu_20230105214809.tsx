...
function MobileMenu({ isOpen, closeMobileMenu }: Props) {
  return (
    <DialogOverlay
      className="fixed inset-0 isolate dark:bg-background bg-text bg-opacity-95 z-50"
      isOpen={isOpen}
      onDismiss={closeMobileMenu}
    >
      {/* <DialogContent className="h-full flex items-start justify-end flex-col p-6 gap-10 "> */}
      <button onClick={closeMobileMenu} className="absolute p-1  top-2 right-7">
        <Close fontSize="large" />
        <span className="sr-only">Close mobile menu</span>
      </button>
      <nav className="absolute top-1/4 left-6 flex flex-col gap-6 w-3/4 z-10">
        <Link
          href="/home"
